...
export default async function AnimatorPage({ params }: AnimatorPageProps) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {signatureClips.length > 0 && (
        <section className="mt-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {signatureClips.map((clip: { id: string; slug: string; title: string; thumbnailUrl: string | null; anime: { title: string } }) => (
              <a
                key={clip.id}
      ...
      {(animator.incomingRelations.length > 0 || animator.outgoingRelations.length > 0) && (
        <section className="mt-12">
          <h2 className="font-display text-heading-lg text-foreground mb-6">
            Influence Network
          </h2>
          <div className="grid md:grid-cols-2 gap-8">
            {animator.incomingRelations.length > 0 && (
              <div>
                <h3 className="text-lg font-semibold text-foreground mb-4">Mentors</h3>
                <div className="space-y-3">
                  {animator.incomingRelations.map((relation: { source: { id: string; slug: string; name: string; photoUrl: string | null } }) => (
                    <a
                      key={relation.source.id}
                      href={`/animators/${relation.source.slug}`}
                    ...
                    >
                      <div className="w-10 h-10 rounded-full bg-surface-hover overflow-hidden">
                        {relation.source.photoUrl ? (
                          <img
                            src={relation.source.photoUrl}
                            alt={relation.source.name}
                            className="object-cover w-full h-full"
                          />
                        ) : (
            ...
            {animator.outgoingRelations.length > 0 && (
              <div>
                <h3 className="text-lg font-semibold text-foreground mb-4">Students</h3>
                <div className="space-y-3">
                  {animator.outgoingRelations.map((relation: { target: { id: string; slug: string; name: string; photoUrl: string | null } }) => (
                    <a
                      key={relation.target.id}
                      href={`/animators/${relation.target.slug}`}
                      className="flex items-center gap-3 p-3 rounded-card bg-surface hover:bg-surface-hover transition-colors"
                    >
                      <div className="w-10 h-10 rounded-full bg-surface-hover overflow-hidden">
                        {relation.target.photoUrl ? (
                          <img
                            src={relation.target.photoUrl}
                            alt={relation.target.name}
                            className="object-cover w-full h-full"
                          />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center text-foreground-muted">
