...
export async function getPendingClips(params: ModerationQueryParams = {}) {
  ...
  const orderBy =
    params.sortBy === 'trust'
      ? [{ submittedBy: { trustScore: params.sortOrder || 'desc' } }, { createdAt: 'asc' as const }]
  ...
  const userIds = clips.map((c) => c.submittedBy).filter(Boolean) as string[]
  ...
  const data = clips.map((clip) => ({
    ...
    attributions: clip.attributions.map((a) => ({
      animator: a.animator,
      role: a.role,
    })),
  }))
