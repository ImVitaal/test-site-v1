...
export async function getUserCollections(userId: string) {
  ...
  return collections.map((c) => ({
    ...
    thumbnails: c.clips.map((cc) => cc.clip.thumbnailUrl),
...
export async function getCollectionBySlug(slug: string) {
  ...
  return {
    ...
    thumbnails: collection.clips.slice(0, 4).map((cc) => cc.clip.thumbnailUrl),
    createdAt: collection.createdAt,
    user: collection.user,
    clips: collection.clips.map((cc) => ({
      id: cc.clip.id,
      slug: cc.clip.slug,
      title: cc.clip.title,
...
export async function getClipCollectionIds(clipId: string, userId: string) {
  ...
  return collectionClips.map((cc) => cc.collectionId)
}
